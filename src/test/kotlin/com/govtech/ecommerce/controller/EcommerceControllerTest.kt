...
internal class EcommerceControllerTest {
    ...
    fun checkUploadEndPoint() {
        ...
        mockMvc.perform(get("/api/ecommerce/dataset/upload"))
    ...
    fun checkListEndPoint() {
        ...
        every { mockService.listInvoiceByPage() } returns expectedResult
        every { mockService.checkifDBEmpty() } returns 1

        mockMvc.perform(get("/api/ecommerce/dataset/list"))
            .andExpect(status().isOk)
    ...
    fun checkSearchEndPoint() {
        ...
        mockMvc.perform(post("/api/ecommerce/dataset/search")
            .param("query","INV01"))
            .andExpect(status().isOk)
            .andExpect(content().contentType(MediaType.APPLICATION_JSON))
